...
export default class SideTocPane extends React.Component {
  ...
  attatchEvents(editor) {
    this.log(() => "attatchEvents");
    this.updateState();

    const { cm } = editor;
    cm.on("cursorActivity", this.handleCursorActivity);
    cm.on("update", this.handleCmUpdate);
  ...
  updateState = () => {
    ...
    this.log(() => "updateState - this.SetState");
  ...
  handleCmUpdate = (_) => {
    ...
    this.log(() => "handleCmUpdate - callUpdateState");
    this.updateState();
  };
  /*
   *
